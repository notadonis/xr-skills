...
5. [Store Submission Requirements](#stores)
6. [XR Pull Request Review Checklist](#review)
...
## Store Submission Requirements {#stores}

Shipping ends at a review gate, not a build. Budget 1–2 weeks for a first submission.
Policies change frequently — verify the current checklist on the store's developer site
before submitting.

### Meta Horizon Store (Quest)

App Lab merged into the main store in 2024: everything is discoverable, everything is
reviewed against the VRCs (store technical requirements). The categories:

- **Functional:** no crashes, boundary/guardian respected, app pauses and mutes when the
  headset is removed, resumes cleanly, correct behavior through sleep/wake.
- **Performance:** sustained target framerate under real load, not just at the menu.
  Reviewers test longer sessions — thermal throttling failures surface here.
- **Privacy/Data:** complete Data Use Checkup declaring every data type collected
  (mic, hands, eye, spatial data), working privacy policy URL. The most common
  first-submission rejection.
- **Input:** every advertised input mode actually works. If the store listing says hand
  tracking, hand tracking must fully work.
- Age rating via the IARC questionnaire.

### visionOS App Store

Standard App Review plus spatial checks: correct Shared Space behavior, system gestures
respected, no simulated camera motion. Privacy nutrition labels must cover ARKit data
(hands, room). Note: raw eye tracking is never exposed to apps on visionOS — never claim
or design for gaze analytics there.

### Steam (PC VR)

Lightest review of the group. Ship OpenXR, and test against both SteamVR and Meta PC
runtimes — runtime-specific bugs are the usual post-launch complaint.

### Google Play (Android XR)

Standard Play review plus XR quality guidelines. The ecosystem is young; check the
current Android XR quality checklist immediately before submission.
