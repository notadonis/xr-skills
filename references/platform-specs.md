...
- SoC: Snapdragon XR2+ Gen 2
- Display: micro-OLED, ~4K-class per eye, up to 90 Hz
- Tracking: Inside-out 6DoF, hand tracking + eye tracking standard
- Input: Gaze + pinch primary (Vision Pro interaction model). Controllers optional,
  sold separately. Design for hands-first.
- Primary SDKs: Jetpack XR (Compose for XR, SceneCore, ARCore for Jetpack XR) for
  native; Unity via OpenXR Android XR support; WebXR in Chrome
- System compositor manages the render loop for native apps, similar to visionOS
- Key constraint: hands + eyes must carry every interaction. Treat controllers as a
  bonus, never a requirement.
- Budgets: use the standalone Quest 3 budgets as the safe baseline until you profile
  on device. Ecosystem is young — verify SDK surfaces against current Android XR docs
  before writing code.
